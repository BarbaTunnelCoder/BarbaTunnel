...
//Changed to orginal source
// change std::string to std::tstring
// change return buffer from std::string to std::vector<BYTE>
...
void Base64::decode(std::string const& encoded_string, std::vector<BYTE>& ret) 
{
  ...
  while (in_len-- && ( encoded_string[in_] != '=') && is_base64(encoded_string[in_])) {
    ...
    if (i ==4) {
      for (i = 0; i <4; i++)
        char_array_4[i] = (u_char)base64_chars.find(char_array_4[i]);
  ...
  if (i) {
    ...
    for (j = 0; j <4; j++)
      char_array_4[j] = (u_char)base64_chars.find(char_array_4[j]);
...
std::tstring Base64::encode(std::vector<BYTE>* buffer)
{
